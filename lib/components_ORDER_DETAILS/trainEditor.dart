...
// ==========================================
// 2. TRAIN DETAILS EDITOR (FULL CODE)
// ==========================================

class TrainDetailsEditor extends StatefulWidget {
  final String orderId;
  const TrainDetailsEditor({super.key, required this.orderId});

  @override
  State<TrainDetailsEditor> createState() => _TrainDetailsEditorState();
}

class _TrainDetailsEditorState extends State<TrainDetailsEditor> {
  ...
  Widget build(BuildContext context) {
    return _BaseEditorContainer(
      ...
      onAddPressed: _addTrain,
      addButtonLabel: "Add Train",
      child: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('orders').doc(widget.orderId).collection('trains').orderBy('departure_time').snapshots(),
        builder: (context, snapshot) {
          if (!snapshot.hasData) return const Center(child: Padding(padding: EdgeInsets.all(20), child: CircularProgressIndicator()));
          final docs = snapshot.data!.docs;
          if (docs.isEmpty) return const _EmptyPlaceholder(text: "No trains added.", icon: Icons.train);
          
          return ListView.separated(
            shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), padding: const EdgeInsets.all(20),
            itemCount: docs.length,
            separatorBuilder: (_, __) => const SizedBox(height: 20),
            itemBuilder: (context, index) => _TrainRowEditor(key: ValueKey(docs[index].id), docId: docs[index].id, data: docs[index].data() as Map<String, dynamic>, orderId: widget.orderId, availableCities: _availableCities),
          );
        },
      ),
    );
...
class _TrainRowEditor extends StatefulWidget {
  final String orderId, docId;
  final Map<String, dynamic> data;
  final List<String> availableCities;
  const _TrainRowEditor({super.key, required this.orderId, required this.docId, required this.data, required this.availableCities});

  @override
  State<_TrainRowEditor> createState() => _TrainRowEditorState();
}

class _TrainRowEditorState extends State<_TrainRowEditor> {
  ...
  void initState() {
    super.initState();
    _trainNoCtrl.text = widget.data['train_number'] ?? '';
    _depStationCtrl.text = widget.data['departure_station'] ?? '';
    _arrStationCtrl.text = widget.data['arrival_station'] ?? '';
    ...
    if (_signBoardCtrl.text.isNotEmpty) _showTransferDetails = true;
  }

  void _updateField(Map<String, dynamic> updates) {
    FirebaseFirestore.instance.collection('orders').doc(widget.orderId).collection('trains').doc(widget.docId).update(updates);
  }
  ...
  // Calculates Client Price = Internal + Markup
  // Calculates Client Price = Internal + (Internal * Markup%)
  void _onPriceChanged() {
    ...
    _debounce = Timer(const Duration(milliseconds: 800), () {
      _updateField({
        ...
        'markup': markupPercent, // Storing as percentage now
  ...
  Widget build(BuildContext context) {
    return _RowContainer(
      onDelete: () => FirebaseFirestore.instance.collection('orders').doc(widget.orderId).collection('trains').doc(widget.docId).delete(),
      child: Column(
        children: [
          Row(children: [
          ...
          Row(children: [
            Expanded(child: _TextFieldInput(controller: _depStationCtrl, label: "Dep. Station", icon: Icons.train, onChanged: (v) => _onTextChanged('departure_station', v))),
            const SizedBox(width: 8),
            Expanded(child: _TextFieldInput(controller: _arrStationCtrl, label: "Arr. Station", icon: Icons.place, onChanged: (v) => _onTextChanged('arrival_station', v))),
          ]),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(flex: 2, child: _TextFieldInput(controller: _trainNoCtrl, label: "Train No.", icon: Icons.confirmation_number, onChanged: (v) => _onTextChanged('train_number', v))),
            const SizedBox(width: 8),
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                const Text("Train Pricing", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.green)),
          ...
          InkWell(
            ...
            child: Row(
              children: [
                Icon(_showTransferDetails ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down, size: 16, color: Colors.teal),
                const SizedBox(width: 4),
                Text(_showTransferDetails ? "Hide Additional Details" : "Show Additional Details", style: const TextStyle(fontSize: 12, color: Colors.teal, fontWeight: FontWeight.bold)),
              ],
            ),
          ),
          ...
          if (_showTransferDetails) ...[
            ...
            Container(
              ...
              child: Row(children: [
                Expanded(child: _TextFieldInput(controller: _signBoardCtrl, label: "Sign Board", icon: Icons.person_pin, onChanged: (v) => _onTextChanged('signboard', v))),
